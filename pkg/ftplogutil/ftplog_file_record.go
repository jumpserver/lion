...
func ftpLogFileRecord(jmsService *service.JMService) {
	...
	for {
		...
		path, err := GetFileCachePath(&ftpLogObj)
		target, _ := filepath.Rel(GetFileCacheRootPath(), path)
		if err == nil && UploadFtpLogFile(path, target, ftpLogObjId, jmsService) {
...
func UploadFtpLogFile(path string, target string, ftpLogId string, jmsService *service.JMService) (success bool) {
	...
	fileStorage := storage.NewReplayStorage(terminalConf.ReplayStorage)
	var err error
	if fileStorage != nil {
		err = fileStorage.Upload(path, targetPath)
	} else {
		err = jmsService.UploadFTPFile(ftpLogId, path)
	}
	if err == nil {
		if jmsService.FinishFTPLogFileUpload(ftpLogId) {
			err = os.Remove(path)
			if err != nil {
...
func dropTimoutFtpLog(ftpLogMap map[string]model.FTPLog, ftpLogFileMap map[string]model.FTPLog) {
	for ftpLogId := range ftpLogFileMap {
		...
		theTime, _ := time.Parse("2006-01-02 15:04:05 +0000", fmt.Sprintf(`"%s"`, tmpFtpLog.DataStart))
